...
contract Ballot{
    ...
    string memory ballotID,string memory organization,string memory describe) public{
        ...
        config[ballotID].candidateLegal.push(false);
        emit logcandidateApply("申请成功!");
    }
    //审核投票人
    event logcheckVoter(string);
    function checkVoter(uint weight,address vot) public{
        voter[vot].weight = weight;
        emit logcheckVoter("审核完成！");
    }
    //审核候选人
    event logcheckCandidate(string);
    function checkCandidate(address candid) public{
        candidate[candid].canBeVote = true;
    ...
    string[] memory voterEmpNos,string[] memory voterNames,bool[] memory voterLegal){
        voters = Voters;
        voterEmpNos = VoterEmpNos;
        voterNames = VoterNames;
        voterLegal = VoterLegal;
    }
    //查看所有投票
