...
contract Ballot{
    ...
    struct Config{
        ...
        uint startTime;//开始时间
        uint endTime;//结束时间
        Candidate[] candidates;//本场投票候选人
        Candidate[] victors;//本场投票获胜者
    }
    string BallotID;//投票场次ID
    ...
    string memory ballotID,string memory organization,string memory describe) public{
        ...
        config[ballotID].candidates.push(candidate[userID]);
    ...
    function calcRes(string memory ballotID)public{
        ...
        for (uint i = 0; i < config[ballotID].candidates.length; i++) {
            for (uint j = i+1; j < config[ballotID].candidates.length; j++) {
                if(config[ballotID].candidates[j].supportCount>config[ballotID].candidates[i].supportCount){
    ...
    //读取某场投票的候选人
    function getCandidates(string memory ballotID)public view returns(Candidate[] memory) {
        return config[ballotID].candidates;
    }
    //读取某场投票获胜的候选人
    function getVictors(string memory ballotID)public view returns(Candidate[] memory) {
        return config[ballotID].victors;
    }
    //查看投票人信息
    function getVoterInfo(address userID) public view returns(Voter memory){
        return voter[userID];
    }
    //查看候选人信息
    function getCandidateInfo(address userID) public view returns(Candidate memory){
    ...
    function getBallots()public view returns(Config[] memory){
        return ballots;
    }
}