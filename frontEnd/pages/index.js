...
const index = (props) => {
  ...
  const preMsgList = props.status === 'success' ?
    ['[no_ansi]<span style="color:#3DC37F">平台已经启动!</span> 可访问 <a href="http://localhost:7001" target="_blank">http://localhost:7001</a>']
    .concat(['[no_ansi] 点击进入 <a href="http://localhost:3000/account" target="_blank">核心功能测试页面</a>'])
    .concat(['等待下一步操作...'])
  ...
  const stopCluster = async () => {
    setMsg([...msgList, '尝试停止平台...']);
    const res = await stopETH();
    try {
      const data = await res.json();
      if (!data.data) {
        setMsg([...msgList, '测试平台已停止']);
        setStatus('warning');
        setNodeList([]);
        setSignerList([]);
        setAccountList([]);
  ...
  const restartCluster = async () => {
    setMsg([...msgList, '尝试重启平台...']);
    const res = await restartETH();
    try {
      const data = await res.json();
      if (!data.data) {
        setMsg([...msgList, '测试平台已成功重启!需要等待大约2分钟直至启动完成', '等待下一步操作...']);
        setStatus('success');
      } else {
        setMsg([
          ...msgList,
  ...
  const ExecModel = () => {
    ...
    const [callerPwd, setCallerPwd] = useState('');
    const handleOk = async (name,method,callType,caller,callerPwd,paras) => {
      
      setExecOpen(false);
      console.log(eval(paras))
      console.log(typeof(eval(paras)))
      console.log(name,method,paras,callType,caller,callerPwd)
      setMsg([...msgList, '开始测试合约...']);
      const data = await execContractMethod(name,method,callType,caller,callerPwd,eval(paras));
      
      const rst = await data.json();

      if (rst.status === 'success') {
        setMining(true);
        setMsg([...msgList, '执行结果：',JSON.stringify(rst.data)]);
      } else {
    ...
    return (
      <Modal
        ...
        onOk={() => handleOk(name,method,callType,caller,callerPwd,paras)}
        okButtonProps={{ disabled: name.trim().length === 0||method.trim().length === 0||caller.trim().length===0||callerPwd.trim().length===0}}
      >
        <Alert showIcon message="合约测试之前要先部署，匿名函数无法测试" type="info"/>
        <div style={{
          marginTop: 20
        }}>
          <span
  ...
  useEffect(() => {
    ...
    socket.on('onLoad', async () => {
      ...
      setMsg([...nowMsgList, `[no_ansi]平台已可用, 可访问 <a href="http://localhost:7001" target="_blank">http://localhost:7001</a>`, '等待下一步操作...']);
      setStatus('success');
    });
                ...
                extra={
                  <Button.Group>
                    {NodeList.length === 0 && status === 'default' ? <Button
                      type={'primary'}
                      onClick={async () => {
                        setMsg([...msgList, `正在启动平台, 从节点启动数${startNodes}...`]);
