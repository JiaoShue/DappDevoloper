...
export default class ContractHandler extends Controller {
  ...
  public async execContractMethod() {
    const { ctx } = this;
    const { name, method, callType , caller , callerPwd, params } = ctx.request.body;
    if (!(name && method && caller && callerPwd)) {
      ctx.body = {
        status: 'error',
        data: '缺少参数',
      };
    ...
    const result = await ctx.service.contracts.executeContract(name, method, callType, caller , callerPwd, params);
    ctx.body = result;
    return;
  }
  ...
  public async sendTransaction() {
    ...
    if (!(opt && opt.sourceAccount && opt.txPassword && opt.distAccount && opt.amount && opt.data)) {
      ctx.body = {
        status: 'error',
        data: '请输入完整转账信息',
      };
    ...
    try {
      const rst = await ctx.service.contracts.sendTransaction(opt.sourceAccount, opt.txPassword, opt.distAccount, opt.amount, opt.data);
      ctx.body = {
        status: 'success',
        data: rst,
      };
  ...
  public async submitInfo() {
    const { ctx } = this;
    const opt = ctx.request.body;
    if (!(opt && opt.sourceAccount && opt.txPassword && opt.data)) {
      ctx.body = {
        status: 'error',
        data: '请填入完整信息',
      };
    ...
    try {
      const rst = await ctx.service.contracts.submitInfo(opt.sourceAccount, opt.txPassword, opt.data);
      ctx.body = {
        status: 'success',
        data: rst,
      };
  ...
  public async getBalance() {
    ...
    try {
      const rst = await ctx.service.contracts.getBalance(account);
      ctx.body = {
        status: 'success',
        data: rst,
      };
