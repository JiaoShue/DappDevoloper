...
export default class Contracts extends Logger {
  ...
  public async executeContract(name: string, method: any, callType: string, caller: string, callerPwd: string, params: any[]) {
    if (!Contracts.web3) {
      Contracts.web3 = new Web3('http://localhost:8545');
    }
    let Contract;
    ...
    await this.ctx.service.gethApi.unlockAccount(caller, callerPwd)();
    if (callType === 'send') {
      if (!params)  {
        return await Contract.methods[method]()[callType]({ from: caller })
        .then(res => {
          return {
            status: 'success',
        ...
        return await Contract.methods[method](...params)[callType]({ from: caller })
        .then(res => {
          return {
            status: 'success',
            data: res,
  ...
  public async getBalance(account: string) {
    ...
    const balance = await Contracts.web3.eth.getBalance(account);
    return balance;
  }
  public async sendTransaction(sourceAccount: string, txPassword: string, distAccount: string, amount: string, data: string) {
    if (!Contracts.web3) {
      Contracts.web3 = new Web3('http://localhost:8545');
    }
    await this.ctx.service.gethApi.unlockAccount(sourceAccount, txPassword)();
    const txDetail = await Contracts.web3.eth.sendTransaction({ from: sourceAccount, to: distAccount, value: Contracts.web3.utils.toWei(amount, 'ether'), data: data});
    return txDetail;
  }

  public async submitInfo(sourceAccount: string, txPassword: string, data: string) {
    if (!Contracts.web3) {
      Contracts.web3 = new Web3('http://localhost:8545');
    }
    await this.ctx.service.gethApi.unlockAccount(sourceAccount, txPassword)();
    const txDetail = await Contracts.web3.eth.sendTransaction({ from: sourceAccount, to: sourceAccount, value: 0 ,data: data})
    return txDetail;
  }
          ...
          exec(`solcjs --bin --abi --optimize -o bin ${target}`, async err => {
            if (err) {
              ...
              myContract
                ...
                .send({
                  from: starter,
                  gas: estimateGas * 5 > 200000000 ? 200000000 : estimateGas * 5
                })
                .on('error', err => {
